...
void displayConsoleBox() {
  ...
  textAlign(CENTER);
  textSize(25);
  text("Press Space to see analysis.",width - (width - picWidth)/2 + 50,height - 27);
}
...
void displayAnalysisResults() {
  ...
  for (float i = 0; i < 10; i++) {
    float h = (10 - i)*(height - 150)/10 + 100;
    ...
    if (img != null) {
      text(String.format("%4.0f",(float)i*maxCount/10),width - picWidth - 40,h);
    } else {
      text(String.format("%4.0f",i*10),width - picWidth - 40,h);
    }
  }
  ...
  for (int i = 0; i < counts.length; i++) {
    ...
    if (img != null) {
      ...
      //textSize(5);
      //text(counts[i],i*(width - picWidth - 50)/counts.length,height - 100 - (float)counts[i]*(height - 150)/maxCount);
      
      rect(
        i*(width - picWidth - 50)/counts.length,
        height - 50,
        (width - picWidth - 50)/counts.length,
        -(float)counts[i]*(float)(height - 150)/maxCount
      );
    }
  }
