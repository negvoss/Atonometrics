...
void analyzeImage() {
  ...
  for (color p:img.pixels) {
    if (red(p) > green(p) && red(p) > blue(p)*2) {
      redTotal++;
    }
    if (green(p) > red(p) && green(p) > blue(p)*2) {
      greenTotal++;
    }
    if (blue(p)*2 > red(p) && blue(p)*2 > green(p)) {
      blueTotal++;
    }
    
    println(red(p) + green(p) + blue(p));
    //writeToConsole(Float.toString(red(p) + green(p) + blue(p)));
  ...
  println(redTotal,greenTotal,blueTotal,whiteTotal,blackTotal);
  
  writeToConsole(redTotal + " " + greenTotal + " " + blueTotal + " " + whiteTotal + " " + blackTotal);
}
